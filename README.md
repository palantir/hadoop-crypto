...
`crypto-core` and `hadoop-crypto` use the crypto providers supplied by the JDK. The legacy
`ApacheCiphers` and `ApacheCtrDecryptingSeekableInput` APIs are available from the optional
`com.palantir.hadoop-crypto2:apache-crypto` artifact, which carries the Apache Commons Crypto dependency.
